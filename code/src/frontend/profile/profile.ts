...
import { Component, computed, effect, signal, WritableSignal, DestroyRef, inject } from '@angular/core';
...
export class Profile {
	...
	private readonly routeUsername = signal('');
	...
	constructor(
		...
		.subscribe(paramMap => this.routeUsername.set((paramMap.get('username') ?? '').trim()));

		effect(() => {
			const currentUserUsername = this.currentUser()?.username;

			const username = (this.routeUsername() || currentUserUsername || '').trim();
